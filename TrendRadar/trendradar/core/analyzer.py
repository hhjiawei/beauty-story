...
计算新闻热度权重（决定哪条新闻排前面）
关键词匹配统计：按配置的关键词分组筛选新闻
支持多种运行模式：日报 / 增量 / 实时榜单
统一数据格式：把杂乱的新闻转成标准结果（给搜索 / 报告用）
RSS 与热榜数据通用：一套逻辑兼容两种数据源
标记新增新闻：区分新热点 / 旧热点



二、逐函数详细解析
1. calculate_news_weight（新闻权重计算）
这是排序的核心算法！ 决定哪条新闻优先展示。
计算逻辑
新闻最终得分 = 排名权重 + 频次权重 + 热度权重
排名权重：排名越靠前，分数越高（1~10 名）
频次权重：新闻出现次数越多，分数越高
热度权重：多次进入高排名的新闻，额外加分
特点
所有分数归一化到 0~100，方便加权计算
可通过配置调整三个维度的比重（默认：排名 60%、频次 30%、热度 10%）
你的搜索 Skill 返回结果的排序规则，完全依赖这个函数

2. format_time_display（时间格式化）
纯工具函数：
把抓取的时间（HH:MM）转换成友好展示格式
支持「首次出现时间～最后出现时间」的区间展示
无复杂逻辑，仅用于界面 / 语音播报美化

3. count_word_frequency（热榜词频统计，核心主函数）
整个模块最复杂、最核心的函数，TrendRadar 的搜索 / 统计全靠它。
核心功能
关键词匹配
支持 必须词（必须包含）+ 普通词（包含其一）
支持过滤词、全局过滤词
三种运行模式
daily：当日所有新闻汇总（生成日报）
incremental：增量模式（只统计新增新闻，用于实时推送）
current：当前榜单模式（只看最新一轮抓取的新闻）
去重 + 标记新增：自动去重新闻，标记是否为新热点
分组统计：按关键词分组，统计每个关键词的新闻数量
排序输出：按权重排序，限制最大展示数量
输入输出
输入：抓取的热榜数据、关键词配置、过滤词
输出：按关键词分组的标准统计结果（你的搜索 Skill 直接用这个结果）

4. count_rss_frequency（RSS 词频统计）
为 RSS 数据源定制的统计函数，和热榜统计格式完全统一。
特点
兼容 RSS 数据结构（发布时间、RSS 源名称）
用「发布时间」模拟排名（最新 = 排名靠前）
支持关键词匹配、新增标记、去重
输出格式和热榜完全一致，上层搜索 / 展示无需修改代码

5. convert_keyword_stats_to_platform_stats（分组维度转换）
工具函数：
把「按关键词分组」的结果 → 转换成「按平台分组」
比如：从「人工智能：10 条新闻」→「微博：5 条、知乎：3 条」
用于多维度数据分析，搜索 Skill 可按需使用
