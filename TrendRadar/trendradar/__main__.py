...
"""
主要方法流程：
__init__() → 加载配置、初始化存储、设置代理
run() → 主执行流程：爬取 → 分析 → 生成报告 → 发送通知
_crawl_data() → 爬取热榜数据，保存到 SQLite
_crawl_rss_data() → 抓取 RSS 订阅源
_run_analysis_pipeline() → 分析流水线（关键词匹配/AI筛选 → AI分析 → 翻译 → HTML生成）
_send_notification_if_needed() → 推送到多渠道（飞书、钉钉、企业微信、Telegram、邮件等）
"""
# === 主分析器 ===
class NewsAnalyzer:
    """新闻分析器"""
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.proxy_url = None
        self._setup_proxy()
        self.data_fetcher = DataFetcher(self.proxy_url) # 专门用于从 newsnow.busiyi.world 这个新闻聚合 API 抓取数据。
