...
这是 TrendRadar 项目的核心数据采集模块，专门负责批量抓取 RSS 订阅源、解析内容、过滤过期文章、输出标准化数据，是整个趋势雷达的「数据源入口」

核心作用
    批量抓取 RSS 新闻 / 文章源（如知乎、博客、科技媒体 RSS）
    统一解析 不同格式的 RSS/Atom XML 数据
    按时间过滤 只保留最新内容（新鲜度控制）
    输出标准格式 给后续的趋势分析、搜索功能使用
    防反爬：请求间隔、随机抖动、请求头伪装
"""
...
from .parser import RSSParser # RSS解析器 解析 XML/JSON 格式的 RSS
from trendradar.storage.base import RSSItem, RSSData # 标准数据模型 项目统一的数据结构（单篇文章 / 抓取结果）
from trendradar.utils.time import get_configured_time, is_within_days, DEFAULT_TIMEZONE


@dataclass # 用 @dataclass 简化数据类，专门存储每个 RSS 源的配置
class RSSFeedConfig:
    """RSS 源配置"""
    id: str                     # 源 ID
    name: str                   # 显示名称
...
class RSSFetcher:
    ...
    def __init__(
        self,
        feeds: List[RSSFeedConfig],  # 所有RSS源配置
        request_interval: int = 2000,  # 请求间隔（毫秒）
        timeout: int = 15,  # 请求超时
        use_proxy: bool = False,  # 代理
        proxy_url: str = "",
        timezone: str = DEFAULT_TIMEZONE,  # 时区（上海）
        freshness_enabled: bool = True,  # 开启新鲜度过滤
        default_max_age_days: int = 3,  # 默认保留3天内内容
    ):
        """
        初始化抓取器
    ...
    def _create_session(self) -> requests.Session:
        ...
        session.headers.update({ # 伪装请求头，避免被反爬拦截
