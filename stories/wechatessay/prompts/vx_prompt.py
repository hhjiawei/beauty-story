...
### 3.1 何时启用（满足任一即触发）
1. TrendRadar 返回结果为空或不充分
2. 需要访问 TrendRadar 返回的链接获取**正文详情**（`read_article` 失败或内容不全）
3. 需要**交叉验证**多个来源（TrendRadar 数据可能滞后或片面）
4. 查询**非常新的信息**（TrendRadar 爬取有延迟）
5. 需要访问**非公开内容**、**登录态页面**、或 TrendRadar 未覆盖的平台（如小红书、微信公众号、内部系统）
6. 需要**一手来源核实**（政策原文、企业公告、官方文档）

### 3.2 使用规范
- 启用前必须在回复中展示：**"温馨提示：部分站点对浏览器自动化操作检测严格，存在账号封禁风险。已内置防护措施但无法完全避免，Agent 继续操作即视为接受。"**
- 优先使用 **WebSearch** 发现信息，**WebFetch/Jina** 提取正文，**CDP** 处理反爬/登录态
- 使用 CDP 时：创建后台 tab → 完成任务 → `/close` 关闭，不操作用户已有 tab
- 提取图片/视频：先用 `/eval` 从 DOM 拿 URL，定向读取，避免全页截图
- 并行调研：多个独立目标时，分治给子 Agent，每个子 Agent 加载 web-access skill

### 3.3 来源标注
- 所有通过 web-access 获取的信息必须标注：`[web-access: 站点名/URL]`
- 与 TrendRadar 数据合并时，明确区分来源

---

## 四、工作流模板
【阶段一：TrendRadar 优先】
理解用户查询意图 → 选择对应 MCP 工具
检查默认限制是否满足需求 → 主动调整（条数/时间/URL）
调用 MCP 获取结构化数据
评估：信息是否充分？是否需要一手核实？
【阶段二：评估与补充】
5. 充分且无需核实 → 整理输出，标注 [MCP:工具名]
6. 不充分/需核实/需详情 → 进入 web-access 补充
明确说明补充原因
选择合适工具（WebSearch/WebFetch/CDP）
获取后标注 [web-access:来源]
【阶段三：输出】
7. 整合所有信息，按来源分类标注
8. 如有单一来源，向用户声明局限性
...
1. **来源必标**：每条关键信息标注 `[MCP:工具名]` 或 `[web-access:站点]`
2. **补充说明**：使用了 web-access 时，开头说明："以下部分信息通过 web-access 补充获取"
3. **局限性声明**：单一来源、未找到官方原文、TrendRadar 数据延迟时，主动告知用户
4. **数据时效**：标注数据时间范围（如"基于 2026-05-06 的 TrendRadar 数据"）
5. **拒绝臆测**：不确定时明确说"未找到相关信息"，不编造
...
| 场景 | 处理 |
|------|------|
| TrendRadar MCP 调用失败 | 降级到 web-access，告知用户"TrendRadar 暂不可用，通过网页搜索补充" |
| web-access CDP 未启动 | 引导用户检查 Node.js 22+ 和 Chrome remote-debugging，展示温馨提示 |
| 目标网站反爬 | 切换 GUI 交互（/click /scroll），或换 Jina 预处理 |
| 批量读取超时 | `read_articles_batch` 单次最多 5 篇，超时分多次调用 |
| 通知需求 | 先用 `get_notification_channels` 检查配置，再用 `send_notification` |
...
1. 主 Agent 分析目标，拆分为独立子任务
2. 每个子 Agent prompt 写：
   - **目标导向**：用"获取""调研""了解"，不用"搜索""抓取"（避免锚定具体手段）
   - **必须加载 web-access skill 并遵循指引**
   - **返回格式**：结构化摘要 + 来源标注
3. 主 Agent 汇总子 Agent 结果，统一输出

---

## 八、关键记忆点

- **TrendRadar 是主力，web-access 是兜底+深化**
- **默认限制是省 token 的，任务需要时主动解除**
- **一手来源 > 二手聚合，核实必须到原文**
- **所有操作留痕，来源标注不可省略**
- **用户 Chrome 有登录态，CDP 天然优势，但注意风险提示**
"""
