...
**from deepagents import create_deep_agent 我说的是这个create_deep_agent 实现的智能体agent 详情请查看https://docs.langchain.com/oss/python/deepagents/overview 网站 获取deepagents相关信息，不允许使用你自创的**
**Memory 机制方面， 实现了双层记忆系统。短期记忆是当前对话的消息列表，用 类似于java中LinkedHashMap 做 FIFO 淘汰；长期记忆把用户偏好、项目事实等信息持久化到本地 JSON 文件中，每次新会话时根据当前输入做混合检索（BM25 关键词 + 语义相似度），把最相关的记忆注入系统提示词。 RAG 模块方面，通过 Ollama 或远程 API 向量化，存到 SQLite。检索时用混合打分——语义相似度 + 关键词匹配 + 代码块类型加权（方法级权重最高）+ 双命中奖励，防止单个大文件霸占结果。**
**导包时，如果是自定义内容 需要写成 类似from wechatessay.agents.memory_manager import get_memory_manager  就是前面要加 wechatessay. **
**mcp我希望可以写的标准化一些，例如使用一个json文件，里面存mcp相关配置 然后导入MultiServerMCPClient中,最后拼接成total_tool在给create_deep_agent**
**tool  mcp  memories   backend 必须根据create_deep_agent 进行编程**
**所有工具最后有我亲自搭配 你只需要准备好工具代码**
**向量化使用ollama中的向量模型**
**你生成的代码就是用于生产环境**
**skills保存在backends的skills中，请你查找一些和微信公众号文章创作相关的skill放于skills目录 例如humanizer-zh 以及 wechat-post-craft**
**每个节点各有一个create_deep_agent，搭配的tool skill等由我这里分配**
**state中需要有中文描述 让我知道字段干嘛用的**
**write_node节点的写作内容是一次全部写完的，我担心随着文章写作，后面效果会变差，所以这里希望遍历每段大纲content_segments的ContentSegment，每次完成一段写作， 每写作完一段，让用户查看是否满意，并且 为了增加每段写作的质量，和前后连贯性，要把前一段和后一段的大纲也提供出来给agent，让agent每次生成内容时，有一个前后认知，并充分利用ArticlePlotNode、ArticleBlueprintNode、ArticleSearchNode的相关信息，例如哪里可以引用，哪里可以做补充等，请重新设计write-node、prompt、graph等，如果是局部修改，请标明改哪里，方便我修改**
# 现有的state目前是： 
