...
- **本章含第 1 段时**：先过阶段零（读料取证——不确定的年份/人名/官职先联网核实，取证在动笔前；按篇级总卡「驱动分型＋结构选型」定调；对施工卡弹药清单逐条斟酌，与史实或声口不合的宁可弃用）与阶段一（声音卡以篇级总卡「声音素材＋语气基调」定稿：绰号—特征—事迹对照沿用 N2 挂载口径、签名句式只服务节奏不承担事实；朝代称呼随语境自然换用——禁用全篇统一的"我X"领属词自称；钩子卡按开场卡附加字段与篇级总卡「开头前置」施工，开场之问必须是"为什么"句式、最后一句落在正题入口）；
- **后续章节**：沿用既有声音卡，与前一章末段声口对齐，不另起炉灶、不中途改称呼策略；
- **每段过四道引擎**：①心理OS与场景脑补（只填史书留白，不改史实骨架，必挂声明牌"或许/可以想象/估计"）→ ②弹药投放（施工卡弹药清单已由 N2 按选配五问挂载并标推荐度——写作层照单兑现、不另造新弹；仅现场与声口/史实冲突才撤换，逐条写进「技法弹药调整声明」）→ ③反讽结构（材料有真实"自信→翻车"弧线才用，亮锤 ≤ 搭靶段 1/5）→ ④因果链校验三问（因从哪来／果落在哪／与下段怎么咬合——照施工卡「因果接口」焊死）；
