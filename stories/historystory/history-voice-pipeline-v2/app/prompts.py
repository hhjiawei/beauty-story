...
技能接口链（各节点 user 提示词已对齐 skill 契约，2026-09 修订）：
  N1 产出六型资料卡包（C/P/R/B/D/E，validate_cards.py 过闸）
    → N2 消费卡摘要，定案唯一风格并产出《风格施工包》七件（精密雕刻，不出候选）
...
2. **直接定案唯一一套风格**：根据事件特质选定，不出候选、不罗列备选——拿不准的维度在「选定理由」里写清权衡，然后对这套风格做精密雕刻：完整《风格施工包》七件 + 一句「本期语气示例」（写作层定声口的对齐基准，声口向系列记忆中的声口样句对齐）。
...
3. **签名声音系统**：朝代称呼策略（随语境在国号/年号/朝廷/都城/称谓间自然换用——**禁用全篇统一的"我X"领属词自称，AI 感重**）+ 主要人物绰号—特征—事迹对照表（从最炸的事迹反推绰号，正面英雄给尊称不起贬损绰号）+ 称呼降格规则（史料原文引用内保留原称呼）；
...
{{
  ...
  "风格定案": {{
    "风格名": "...",
    "核心气质": "...",
    "选定理由": "为什么是这套（含被否掉的方向及否决理由）",
    "本期语气示例": "...",
    "风格施工包": {{
      "叙事路由": {{"选定": "人物传|事件录|制度论", "判定理由": "...", "骨架清单": "..."}},
      "大纲规划": {{"编年骨架方向": "...", "弹药布点方向": "...", "钩子收尾位置建议": "..."}},
      "签名声音系统": {{"朝代称呼策略": "...", "绰号对照表": [{{"人物": "...", "绰号": "...", "特征": "...", "事迹": "..."}}], "称呼降格规则": "..."}},
...
## 输入：本期风格施工包（N2 定案的唯一风格：「风格施工包」七件 + 本期语气示例）
...
## 输入：本期风格施工包（声口基准：朝代称呼策略 / 绰号对照表 / 称呼降格 / 三轨配比 / 本期语气示例）
{_j(state.get("style_card", {}))}
