...
# 已废除字段(纪律3.4): 弹药归大纲节点从风格层弹药库选配, 资料卡不预判
DEPRECATED_FIELDS = {"弹药潜质", "亮点类型", "弹药"}
...
def check_deprecated_and_empty(cards, report):
    """废弃字段检测(纪律3.4) + 空值字段清理建议(纪律3.10): WARN 级。"""
    deprecated, empty_fields = [], []
    for c in cards:
        cid = str(c.get("卡号", "?"))
        for k in c:
            if k in DEPRECATED_FIELDS:
                deprecated.append(f"{cid}.{k}")
        for k, v in c.items():
            if isinstance(v, str) and v.strip() in {"", "无", "—"} and k not in {"备注"}:
                empty_fields.append(f"{cid}.{k}")
    msgs = []
    if deprecated:
        msgs.append("废弃字段(弹药归大纲节点选配, 请删除): " + "、".join(deprecated[:8]))
    if empty_fields:
        msgs.append("含空值字段(纪律3.10建议直接省略): " + "、".join(empty_fields[:8]))
    if msgs:
        report.warn("废弃字段与空值", " | ".join(msgs))
    else:
        report.passed("废弃字段与空值")
...
def main():
    ...
    check_deprecated_and_empty(cards, report) # WARN
