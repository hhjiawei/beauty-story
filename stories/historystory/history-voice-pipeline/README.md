...
| **Prompt 集中管理** | 全部节点 prompt 收在 `app/prompts.py` 一个文件：人格 L1 常驻前缀在此统一注入，每个 builder 返回 (system, user)，system 首行带 `<!-- NODE:节点id -->` 标记（MockLLM 路由 + 日志审计两用） |
| **六份文档转 skill** | 六份方法论原样转 `skills/*/SKILL.md`（只加 frontmatter 不改正文），另按 §4.1/§4.6 新建 `historical-event-cards`、`narration-auditor` 两个技能 |
| **零依赖降级通道** | `mock_tts`（正弦音合成）+ `MockChatModel`：不配任何 Key、不装 IndexTTS2 也能全链路演示与测试 |
...
2. **项目列表页**：粘贴史料 → 选类型（人物/朝代/事件）→ 创建并启动；
3. **工作台**：流程条实时刷新（SSE），每个闸门看产物 → 放行 / 编辑后放行 / 打回（填意见）；
4. 音频节点逐单元试听、勾选重生；成品页整轨审听、标塌段回退；
5. 归档后 `data/memory/lessons.md` 自动沉淀本任务打回教训，读音词典入库——**下一个任务自动继承**。
...
| test_prompts.py | 人格 L1 常驻注入、节点标记、references 挂载、打回注入、记忆审计 |
...
├── models.py / db.py    SQLModel 表（执行方案第九节）
├── llm.py               OpenAI 兼容客户端 + MockLLM
├── skills_loader.py     技能加载（原文+frontmatter）
├── memory_store.py      人格 L2 系列记忆（lessons/voice_samples）
├── graph/pipeline.py    LangGraph 主图（节点+闸门+回退边）
├── graph/nodes.py       全部节点实现
...
skills/                  8 个技能目录（6 份原文 + 2 份新建）
static/                  index / run / settings / voices 四页（无构建）
data/                    运行时生成：pipeline.db、projects/{id}/产物音频、memory/、refs/
tests/                   5 个测试文件，20 条用例
```
